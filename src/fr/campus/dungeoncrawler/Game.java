...
public class Game {
    ...
    public void playTurn() throws OutOfBoardException {
        ...
        while (playerPosition <= board.getCells().size()) {     // tant que la position du joueur est inférieure ou égale à la taille du board,
            ...
            if (playerPosition >= board.getCells().size()) {            // mais si la position dépasse 64

                playerPosition = board.getCells().size();              // alors la position vaut 64

                System.out.println("Le joueur est sur la case " + playerPosition + "\nContenu de la case : " + board.getCells().get(playerPosition - 1).getCellContent() + "\n\n+--------------+ Gagné ! Fin de partie ! +--------------+\n");
